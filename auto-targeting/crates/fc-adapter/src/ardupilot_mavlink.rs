//! MAVLink-based adapter for real ArduPilot flight controllers.
//!
//! Supports serial, TCP, and UDP transports — anything the `mavlink` crate's
//! `connect()` function accepts:
//!
//! - `serial:/dev/ttyUSB0:115200` — serial (SpeedyBee F405 over UART/USB)
//! - `tcpout:127.0.0.1:5760` — TCP client (connect to SITL's TCP server)
//! - `tcpin:0.0.0.0:14550` — TCP server
//! - `udpin:0.0.0.0:14550` — UDP listener (SITL's default)
//! - `udpout:127.0.0.1:14550` — UDP client
//!
//! ## Usage with real hardware
//!
//! 1. Connect SpeedyBee F405 to Orange Pi via USB or UART.
...
//!    ```
//! 4. Run: `cargo run -p auto-targeting-cli -- --config config.toml`
//!
//! ## Usage with SITL (for testing without real FC)
//!
//! Start SITL with TCP server:
//! ```bash
//! sim_vehicle.py -v ArduPlane -f plane --out=udp:127.0.0.1:14550
//! ```
//!
//! Configure:
//! ```toml
...
//! ## Implementation
//!
//! This adapter shares most of its logic with `SittlMavlinkAdapter`. The
//! only difference is the transport URL — the mavlink crate's `connect()`
//! handles the rest. We delegate to a shared `MavlinkCore` implementation.
...
///
/// This is the production adapter. For SITL-only testing, `SittlMavlinkAdapter`
/// is a simpler wrapper — but this adapter also works with SITL if you use a
/// `tcpout:` or `udpin:` URL.
...
impl ArduPilotMavlinkAdapter {
    ...
    fn spawn_reader(&self, vehicle: Arc<Conn>) {
        ...
        std::thread::spawn(move || {
            info!("MAVLink telemetry reader thread started (ArduPilot)");
            loop {
                match vehicle.recv() {
                    Ok((_header, msg)) => {
...
impl FlightControllerAdapter for ArduPilotMavlinkAdapter {
    async fn connect(&mut self) -> Result<(), FcError> {
        ...
        info!(url = %url, "connecting to ArduPilot via MAVLink");
        ...
        info!("ArduPilot MAVLink connection established: {url}");
        Ok(())
    }
    ...
    async fn disconnect(&mut self) -> Result<(), FcError> {
        ...
        self.vehicle = None;
        info!("ArduPilot MAVLink disconnected");
        Ok(())
    }
...
mod tests {
    ...
    fn adapter_construction_does_not_connect() {
        let cfg = ArduPilotConfig::default();
        let adapter = ArduPilotMavlinkAdapter::new(cfg);
