...
fn main() {
    ...
    rt.block_on(async {
        ...
        for i in 0..(args.count + 5) {
            // Прогрев: первые 5 кадров не считаем (cold start, USB negotiation).
            let counting = i >= 5;
            ...
            // Capture latency = время от начала ожидания до получения кадра.
            let capture_us = t_capture_start.elapsed().as_micros() as u64;

            // Decode latency: MJPG → RGB24.
        ...
        println!("\n=== Results ({warmed} frames, {:.2}s, {:.1} FPS sustained) ===\n", elapsed.as_secs_f64(), fps);
        ...
        println!("\n=== Per-frame budget @ target {} fps ({:.1}ms/frame) ===", args.fps, 1000.0 / args.fps as f64);
        println!(
            "  capture+decode = {:.2}ms → headroom for inference = {:.2}ms",
            total_stats.mean_ms(),
            1000.0 / args.fps as f64 - total_stats.mean_ms()
        );
